...
`;var C=C||Object.getPrototypeOf(customElements.get("ha-panel-lovelace"));var T=C.prototype.html;class cardContent{static generateTitle(e){const i=e.title!==void 0?T` <h1 class="pool-monitor-title">${e.title}</h1> `:T``;return T`${i}`}static generateBody(e,i){if(!i)return T` <div class="warning-message">No sensor data available</div> `;return T`
      <!-- ##### ${i.name} section ##### -->
    ...
    `}static generateCompactBody(e,i){if(!i)return T` <div class="warning-message">No sensor data available</div> `;return T`
      <!-- ##### ${i.name} section ##### -->
      <div class="section-compact" @click=${()=>cardContent._moreinfo(i.entity)}>
        ...
        <div class="pool-monitor-container">
          ...
          <div
            class="cursor-text"
            style="border-${i.side_align}: 5px solid ${e.marker}; text-align:${i.side_align};background-color:transparent; ${i.side_align==="left"?"left:calc(37px + (100% - 42px) * "+parseFloat(i.pct_cursor)/100+");":"right:calc(5px + (100% - 42px) * "+(100-parseFloat(i.pct_cursor))/100+");"};"
          >
            &nbsp; ${i.title} ${i.value} ${i.unit} ${i.separator} ${i.state} &nbsp;
          </div>
          ${i.pct_min!==i.pct_cursor?T`<div
                class="cursor-text"
                style="border-left: 2px solid ${e.colors.hi_low}; border-top: 2px solid ${e.colors.hi_low}; border-bottom: 2px solid ${e.colors.hi_low}; width: 2px; height: 12px; text-align:${i.side_align}; background-color:transparent; ${"left:calc(37px + (100% - 42px) * "+parseFloat(i.pct_min)/100+");"};"
              ></div>`:""}
          ${i.pct_max!==i.pct_cursor?T`<div
                class="cursor-text"
                style="border-right: 2px solid ${e.colors.hi_low}; border-top: 2px solid ${e.colors.hi_low}; border-bottom: 2px solid ${e.colors.hi_low}; width: 2px; height: 12px; text-align:${i.side_align}; background-color:transparent; ${"right:calc(5px + (100% - 42px) * "+(100-parseFloat(i.pct_max))/100+");"};"
              ></div>`:""}
        </div>
        <div class="pool-monitor-container-values">
          <div
    ...
    `}static _moreinfo(e){const i=new Event("hass-more-info",{bubbles:true,composed:true});i.detail={entityId:e};const o=document.querySelector("home-assistant");if(o)o.dispatchEvent(i)}}var P=P||Object.getPrototypeOf(customElements.get("ha-panel-lovelace"));var M=P.prototype.html;console.info(`%c POOL-MONITORING-CARD %c ${f} `,w.title,w.version);class PoolMonitorCard extends P{static cardType=x.cardType;static cardName=x.cardName;static cardDescription=x.cardDescription;static get properties(){return{hass:{},config:{}}}static styles=k;constructor(){super()}render(){const e=this.getConfig();const i=this.processData();const o=e.display.compact?cardContent.generateCompactBody:cardContent.generateBody;if(!i||Object.keys(i).length===0)return M` <ha-card>
      ...
      </ha-card>`;return M` <ha-card><div id="pool-monitor-card">
      ...
      ${Object.values(i).map((i=>{if(i?.invalid)return M`
          ...
          `;return o(e,i)}))}
    </div></ha-card>`}processData(){const e={};const i=this.getConfig();Object.entries(i.sensors).forEach((([i,o])=>{const a=Array.isArray(o)?o:[o];a.forEach(((o,a)=>{const r=`${i}_${a+1}`;e[r]=this.calculateData(i,o.title||this.getTranslatedText("sensor."+i),o.entity,o.min,o.max,o.setpoint,o.step,o.unit,o.icon,o.image_url,o.mode,o.min_limit,o.override_value,o.override,o.invalid)}))}));return e}getTranslatedText(e,i){const o=this.config?.display.language||"en";const a=getTranslation(o,e);return formatTranslation(a,i)}calculateData(e,i,o,a,r,s,n,l,c,d,p,u,m,g,h){const v={};const y=this.getConfig();const _=getSensorConfig(e)||{};v.name=e;v.invalid=h;v.mode=p;v.title=y.display.show_names?i:M`&nbsp;`;v.hide_icon=false;v.is_mdi=false;if(!y.display.show_icons)v.hide_icon=true;else{const i=c||"";const o=d||"";if(i==="hide")v.hide_icon=true;else if(o)v.img_src=o;else if(i&&typeof i==="string"&&i.startsWith("mdi:")){v.is_mdi=true;v.mdi_icon=i}else v.img_src=`https://raw.githubusercontent.com/sierramike/pool-monitor-card/master/resources/${e}.png`}if(!this.hass||!this.hass.states||!this.hass.states[o]){console.warn(`Entity not found: ${o}`);v.value=null;v.entity=o;return v}const f=this.hass.states[o];const x=f.attributes?.display_precision??f.attributes?.precision??this.countDecimals(parseFloat(f.state));const w=parseFloat(f.state);v.value=isNaN(w)?null:Number(w.toFixed(x));v.entity=o;if(y.display.show_last_updated)v.last_updated=this.timeFromNow(f.last_updated);v.unit=y.display.show_units?l||_.unit||"":"";if(g)v.value=m||_.override;v.min_value=a!==void 0&&this.hass.states[a]&&!isNaN(parseFloat(this.hass.states[a].state))?parseFloat(this.hass.states[a].state):v.value;v.max_value=r!==void 0&&this.hass.states[r]&&!isNaN(parseFloat(this.hass.states[r].state))?parseFloat(this.hass.states[r].state):v.value;s=s!==void 0?parseFloat(s):_.setpoint!==void 0?parseFloat(_.setpoint):v.value;n=n!==void 0?parseFloat(n):_.step!==void 0?parseFloat(_.step):.1;const b=Math.max(this.countDecimals(s),this.countDecimals(n));v.setpoint=s;const $=u!==void 0?Number(u):-1/0;const k=Math.max($,s-2*n);const C=Math.max($,s-n);const T=Math.max($,s);const P=Math.max($,s+n);const N=Math.max($,s+2*n);v.setpoint_class=[k.toFixed(b),C.toFixed(b),T.toFixed(b),P.toFixed(b),N.toFixed(b)];v.separator=y.display.show_labels?"-":"";v.color="transparent";if(v.value!==null)v.value=Math.max($,v.value);if(p==="heatflow")if(Number(v.value)<Number(v.setpoint_class[1])){v.state=y.display.show_labels?this.getTranslatedText("state.1"):"";v.color=y.colors.cool}else if(Number(v.value)>=Number(v.setpoint_class[1])&&Number(v.value)<Number(v.setpoint_class[3])){v.state=y.display.show_labels?this.getTranslatedText("state.3"):"";v.color=y.colors.low}else{v.state=y.display.show_labels?this.getTranslatedText("state.5"):"";v.color=y.colors.warn}else if(Number(v.value)<Number(v.setpoint_class[0])){v.state=y.display.show_labels?this.getTranslatedText("state.1"):"";v.color=y.colors.warn}else if(Number(v.value)>=Number(v.setpoint_class[0])&&Number(v.value)<Number(v.setpoint_class[1])){v.state=y.display.show_labels?this.getTranslatedText("state.2"):"";v.color=y.colors.low}else if(Number(v.value)>=Number(v.setpoint_class[1])&&Number(v.value)<Number(v.setpoint_class[2])){v.state=y.display.show_labels?this.getTranslatedText("state.3"):"";v.color=y.colors.normal}else if(Number(v.value)>=Number(v.setpoint_class[2])&&Number(v.value)<Number(v.setpoint_class[3])){v.state=y.display.show_labels?this.getTranslatedText("state.4"):"";v.color=y.colors.normal}else if(Number(v.value)>=Number(v.setpoint_class[3])&&Number(v.value)<Number(v.setpoint_class[4])){v.state=y.display.show_labels?this.getTranslatedText("state.5"):"";v.color=y.colors.low}else if(Number(v.value)>=Number(v.setpoint_class[4])){v.state=y.display.show_labels?this.getTranslatedText("state.6"):"";v.color=y.colors.warn}v.progressClass=e==="temperature"?"progress-temp":"progress";v.pct=Math.max(0,Math.min(98.5,Math.max(0,v.value-(s-3*n))/(6*n)*100)).toFixed(0);v.pct_min=Math.max(0,Math.min(98.5,Math.max(0,v.min_value-(s-3*n))/(6*n)*100)).toFixed(0);v.pct_max=Math.max(0,Math.min(98.5,Math.max(0,v.max_value-(s-3*n))/(6*n)*100)).toFixed(0);v.pct_marker=v.value>v.setpoint?v.pct-12:v.pct-5;v.side_align=v.value>s?"right":"left";v.pct_cursor=v.pct;v.pct_state_step=v.value>s?105-parseFloat(v.pct):parseFloat(v.pct)+5;return v}countDecimals(e){if(e===void 0||e===null)return 0;if(Math.floor(e)===e)return 0;const i=e.toString();if(i.includes("."))return i.split(".")[1].length||0;return 0}timeFromNow(e){const i=new Date(e);const o=Date.now()-i.getTime();const t=(e,i)=>{const o=i===1?"time":"time_plural";const a={[e]:i};return this.getTranslatedText(`${o}.${e}`,a)};const a=Math.floor(o/6e4);const r=Math.floor(a/60);const s=Math.floor(r/24);if(a<1)return t("seconds",0);if(a<60)return t("minutes",a);if(r<24)return t("hours",r);return t("days",s)}getConfig(){return this.config}setConfig(e){const i={display:getDisplayConfig(),colors:getColorConfig()};const o={...e,display:{...i.display,...e.display||{}},colors:{...i.colors,...e.colors||{}},sensors:{}};if(!e.sensors){console.warn(`Legacy configuration detected, please move sensors under "sensors" key`,{config:e});throw new Error('Legacy configuration detected. Please update your setup to define all sensors under the "sensors" key as required by the version 2.0 of the card.')}Object.entries(e.sensors).forEach((([i,a])=>{const r=getSensorConfig(i);const s=Array.isArray(a)?[...a]:[{...a}];if(s.length===0)throw new Error(`Empty sensor array for ${i}`);const n=s.map((e=>({...r,...e,nameDefinedByUser:!!e.name})));n.forEach(((o,a)=>{if(!o.entity)throw new Error(`Missing entity for ${i}[${a}]`);if(o.nameDefinedByUser)o.title=o.name;if(!v.includes(i)){console.warn(`Unsupported sensor type: ${i}`,{sensorType:i,supportedSensors:v,config:e,sensorConfig:o});o.invalid=true}else o.invalid=false}));o.sensors[i]=n}));this.config=o}}customElements.define("pool-monitor-card",PoolMonitorCard);window.customCards=window.customCards||[];window.customCards.push({type:x.cardType,name:x.cardName,description:x.cardDescription});e.PoolMonitorCard=PoolMonitorCard;Object.defineProperty(e,Symbol.toStringTag,{value:"Module"});return e}({});
